...
public class minimax_ke extends AIModule
{
	...
	public void getNextMove(final GameStateModule state)
	{
		...
		{
			if(depth < 5)
				bestMoveAndValue = miniMax(game, depth, -1, game.getActivePlayer());
			depth++;
	...
	private int[] miniMax(final GameStateModule game, int depth, int selectedColumn, int player){
		...
		if(game.isGameOver() || legalMove == 0 || depth == 0){
			...
			if(player == 1){
				...
				for(int col = 0; col < game.getWidth() && !terminate; col++){
					
					if(game.canMakeMove(col)){
						
						game.makeMove(col);
						temp = miniMax(game, depth - 1, col, 2);
						currentValue = temp[1];
			...
			/**************player 2 always be minimizer**************/
				...
				for(int col = 0; col < game.getWidth() && !terminate; col++){
					...
					if(game.canMakeMove(col)){
						...
						game.makeMove(col);
						temp = miniMax(game, depth - 1, col, 1);
						currentValue = temp[1];
